...
impl TrainerStates {
    pub fn new() -> Self {
        let game_process = match find_process(offset::GAME_PROCESS_NAME) {
        ...
        let game_module = match find_module_ex(&game_process, offset::GAME_MODULE_NAME) {
    ...
    pub fn no_cool_down(&mut self, toggle: bool) -> Result<bool, String> {
        ...
        let no_cool_down_address = self.game_module.base + offset::NO_COOLDOWN_OFFSET;
        if toggle {
    ...
    pub fn auto_collect_sun(&mut self, toggle: bool) -> Result<bool, String> {
        ...
        let auto_collect_sun_address = self.game_module.base + offset::AUTO_COLLECT_SUN_OFFSET;
        if toggle {
    ...
    pub fn edit_sun_value(&mut self, value: u32) -> Result<bool, String> {
        if let Some(sun_address) = deep_pointer_ex(
            &self.game_process,
            self.game_module.base + offset::SUN_ADDR_BASE,
            &[offset::SUN_ADDR_OFFSET_1, offset::SUN_ADDR_OFFSET_2],
        ) {
    ...
    pub fn edit_coin_value(&mut self, value: u32) -> Result<bool, String> {
        if let Some(coin_address) = deep_pointer_ex(
            &self.game_process,
            self.game_module.base + offset::COIN_ADDR_BASE,
            &[offset::COIN_ADDR_OFFSET_1, offset::COIN_ADDR_OFFSET_2],
        ) {
